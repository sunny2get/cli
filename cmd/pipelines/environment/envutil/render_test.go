...
func TestPrintEnvironmentListJSON_EmitsArray(t *testing.T) {
	out := captureStdout(t, func() {
		require.NoError(t, PrintEnvironmentListJSON([]pipelines.EnvironmentSummary{
			{EnvironmentID: "env-1", Name: "n", LatestVersion: 1, LatestStatus: pipelines.EnvironmentStatusReady},
		}))
	})
